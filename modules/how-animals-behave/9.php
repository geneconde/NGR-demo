<?php 
	...
	if($user->getType() == 2) {
		...
		$sa = $sac->getStudentAnswer($_SESSION['smid'], 'how-animals-behave-qc3-a');
		$answered = ($sa ? 1 : 0 );
	} else $answered = 1;
?>
<!DOCTYPE html>
...
function checkAnswers() { 	
	...
	if (value=="A"){		
			...
			$('.choice1').html('A. <?php echo _("A food’s smell is an important part of how we think it tastes. Stuffy noses have a hard time detecting smell."); ?>');	
			$('.h2cr1').html('<img src="images/others/check.png" /> <?php echo _("Correct! Most foods don’t taste very good if we can’t smell them."); ?>');	
			$('#qaAnswer').fadeIn();			
	} else if (value=="B"){
			$('.h2cr1').css('color', 'red');	
			$('.choice1').html('B. <?php echo _("A food’s smell is not an important part of how we think it tastes. Stuffy noses have a hard time detecting smell."); ?>');
			$('.h2cr1').html('<img src="images/others/wrong.png" /> <?php echo _("Not quite. Smell is an important part of what we think of as taste."); ?>');
			$('#qaAnswer').fadeIn();	
	} else if (value=="C"){
			$('.h2cr1').css('color', 'red');	
			$('.choice1').html('C. <?php echo _("A food’s smell is an important part of how we think it tastes.  Stuffy noses detect smells better than clear noses."); ?>');	
			$('.h2cr1').html('<img src="images/others/wrong.png" /><?php echo _("Read carefully... Stuffy noses don’t detect smells very well."); ?>');		
			$('#qaAnswer').fadeIn();	
	} else if(value == 'D') {  
			$('.h2cr1').css('color', 'red');	
			$('.choice1').html('D. <?php echo _("A food’s smell is not an important part of how we think it tastes. Stuffy noses detect smells better than clear noses."); ?>');	
