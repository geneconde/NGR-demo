<?php 
	...
	if($user->getType() == 2) {
		...
		$sa = $sac->getStudentAnswer($_SESSION['smid'], 'solar-power-qq5-a');
		$answered = ($sa ? 1 : 0 );
	} else $answered = 1;
?>
...
#container { width: 100%; text-align: center; position: absolute; }
		...
		back.click(function() {
			if($('.img-holder').is(':visible')) {
				...
				answer.fadeOut(function() {
					...
					$('.add-text').text('<?php echo _("Quick Check #4"); ?>'); 
					answer1 = "";
				});
			}
		});
		...
		check.click(function(e){
			...
			if(nullAnswer == true) {
				...
				$('.img-holder').fadeOut(function(){ 
					...
					/* End checking */
					if(answered == 0) {
						saveAnswer('solar-power-qq5-a', ans1);
						saveAnswer('solar-power-qq5-b', ans2);
						saveAnswer('solar-power-qq5-c', ans3);
						saveAnswer('solar-power-qq5-d', ans4);
						saveAnswer('solar-power-qq5-e', ans5);
						answered = 1;
					} // end saving to db
					//alert(answer1 +' '+ answered);
