...
class jqgrid
{
	...
	function render($grid_id)
	{
		...
		if ($this->actions["rowactions"] !== false)
		{
			...
			$actions_var = _('Actions');
			if (!$defined)
			{
				$this->options["colNames"][] = $actions_var;
		...
		{
			$act_links = array();
			$edit_this_row = _("Edit this row");
			$save_this_row = _("Save this row");
			$restore_this_row = _("Restore this row");
			$reload_grid = _("Reload Grid");
			
			### P-START ###
			if ($this->internal["actionicon"] === true) // icons as action links
			{
				if ($this->actions["edit"] !== false)
					$act_links[] = "<a class=\"ui-custom-icon ui-icon ui-icon-pencil\" title=\"$edit_this_row\" href=\"javascript:void(0);\" onclick=\"jQuery(this).dblclick();\"></a>";
				...
				$out .= ",'gridComplete': function()
							{
								...
								for(var i=0;i < ids.length;i++)
								{
									...
									se = '<a class=\"ui-custom-icon ui-icon ui-icon-disk\" title=\"$save_this_row\" href=\"javascript:void(0);\" onclick=\"jQuery(\'#{$grid_id}_ilsave\').click(); if (jQuery(\'#$grid_id\').saveRow(\''+cl+'\') || jQuery(\'.editable\').length==0) { jQuery(this).parent().hide(); jQuery(this).parent().prev().show(); ". addslashes($reload_after_edit)." }\"></a>';
									ce = '<a class=\"ui-custom-icon ui-icon ui-icon-cancel\" title=\"$restore_this_row\" href=\"javascript:void(0);\" onclick=\"jQuery(\'#{$grid_id}_ilcancel\').click(); jQuery(\'#$grid_id\').restoreRow(\''+cl+'\'); jQuery(this).parent().hide(); jQuery(this).parent().prev().show();\"></a>';
						
									// for inline add option
									if (ids[i].indexOf('jqg') != -1)
									{
										se = '<a class=\"ui-custom-icon ui-icon ui-icon-disk\" title=\"$save_this_row\" href=\"javascript:void(0);\" onclick=\"jQuery(\'#{$grid_id}_ilsave\').click(); \">Save</a>';
										ce = '<a class=\"ui-custom-icon ui-icon ui-icon-cancel\" title=\"$restore_this_row\" href=\"javascript:void(0);\" onclick=\"jQuery(\'#{$grid_id}_ilcancel\').click(); jQuery(this).parent().hide(); jQuery(this).parent().prev().show();\">Cancel</a>';
			...
			{
			if ($this->actions["edit"] !== false)
				$act_links[] = "<a title=\"$edit_this_row\" href=\"javascript:void(0);\" onclick=\"jQuery(this).dblclick();\">Edit</a>";
			...
			$act_links = implode(" | ", $act_links);
				...
				$out .= ",'gridComplete': function()
						...
						var ids = jQuery('#$grid_id').jqGrid('getDataIDs');
							...
							// il_save, ilcancel, iledit are clicked for inlineNav button reset
							se = ' <a title=\"$save_this_row\" href=\"javascript:void(0);\" onclick=\"jQuery(\'#{$grid_id}_ilsave\').click(); if (jQuery(\'#$grid_id\').saveRow(\''+cl+'\') || jQuery(\'.editable\').length==0) { jQuery(this).parent().hide(); jQuery(this).parent().prev().show(); ". addslashes($reload_after_edit)." }\">Save</a>'; 
							ce = ' | <a title=\"$restore_this_row\" href=\"javascript:void(0);\" onclick=\"jQuery(\'#{$grid_id}_ilcancel\').click(); jQuery(\'#$grid_id\').restoreRow(\''+cl+'\'); jQuery(this).parent().hide(); jQuery(this).parent().prev().show();\">Cancel</a>'; 
							
							// for inline add option							
							if (ids[i].indexOf('jqg') != -1)
							{
								se = ' <a title=\"$save_this_row\" href=\"javascript:void(0);\" onclick=\"jQuery(\'#{$grid_id}_ilsave\').click(); \">Save</a>'; 
								ce = ' | <a title=\"$restore_this_row\" href=\"javascript:void(0);\" onclick=\"jQuery(\'#{$grid_id}_ilcancel\').click(); jQuery(this).parent().hide(); jQuery(this).parent().prev().show();\">Cancel</a>'; 
								jQuery('#$grid_id').jqGrid('setRowData',ids[i],{act:'<span style=display:none id=\"edit_row_{$grid_id}_'+cl+'\">'+be+'</span>'+'<span id=\"save_row_{$grid_id}_'+cl+'\">'+se+ce+'</span>'});
							}
							else
								jQuery('#$grid_id').jqGrid('setRowData',ids[i],{act:'<span id=\"edit_row_{$grid_id}_'+cl+'\">'+be+'</span>'+'<span style=display:none id=\"save_row_{$grid_id}_'+cl+'\">'+se+ce+'</span>'});
					...
					$out .= "
							...
							jQuery('#$grid_id').editRow(id, true, function()
																	...
																	function()
																				...
																				function()
																				{
																					// force $reload_grid after inline save
																					$reload_after_edit
																				},null,
																				function()
																				{
	...
	?>
		...
		"addParams":{
						"addRowParams":
						{
							...
							{
								// but $reload_grid, to work properly			
