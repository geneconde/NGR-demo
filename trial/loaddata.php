...
$subid = $sub->getID();
$sub_teacher = $sub->getTeachers();
$sub_student = $sub->getStudents();
...
$grid->addColumn('grade_level', 'Grade Level', 'integer');
$grid->addColumn('type', 'Role Type', 'integer' , fetch_pairs($mysqli,'SELECT type_id, type_name FROM user_type WHERE type_id NOT IN ("3","1")'), false);
$grid->addColumn('teacher_id', 'Teacher', 'integer' , fetch_pairs($mysqli,"SELECT user_ID, first_name FROM users WHERE type = 0 AND subscriber_id = '" . $subid . "'"), false);    
...
//$result = $mysqli->query("SELECT * FROM users WHERE subscriber_id = '" .$subid. "'");
 $result = $mysqli->query("SELECT * FROM users WHERE subscriber_id = '" .$subid. "' AND user_ID NOT IN ('".$userid."')");
$mysqli->close();
