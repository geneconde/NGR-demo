...
if($sub->getStudents() != $student_count) {
	...
	$grid->addColumn('type', 'Role Type', 'string' , fetch_pairs($mysqli,'SELECT type_id, type_name FROM user_type WHERE type_id NOT IN ("3", "1", "2")'),true);  
}
...
//$result = $mysqli->query("SELECT * FROM users WHERE subscriber_id = '" .$subid. "'");
 $result = $mysqli->query("SELECT * FROM users WHERE subscriber_id = '" .$subid. "' AND user_ID NOT IN ('".$userid."') AND type = 0 AND is_deleted = 0");
$mysqli->close();
